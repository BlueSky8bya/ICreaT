...
class _CheckBoxButtonState extends State<CheckBoxButton> {
  ...
  Color? get _fillColor {
    if (_isDisabled) return context.bgPrimary;
    if (_isChecked) return context.bgBrand;
    return context.bgPrimary;
  }

  // border
  Border? get _border {
    if (_isDisabled) return Border.all(color: context.borderPrimary);
    if (_isChecked) return Border.all(color: context.bgBrand);
    if (_isHover) return Border.all(color: context.borderPrimaryHoverPressed);
    return Border.all(color: context.borderPrimary);
  }

  // icon Color
  Color get _iconColor {
    if (_isDisabled) return context.borderPrimary;
    if (_isChecked) return context.bgPrimary;
    if (_isHover) return context.bgPrimary;
