...
``` python
    ...
    timestamp= openapi.Parameter('timestamp', openapi.IN_QUERY, description='Unix epoch 초단위 (ex. 1672498800)', required=True, type=openapi.TYPE_STRING)
    csvfile= openapi.Parameter('csvfile', openapi.IN_FORM, description='전송될 워치 내의 데이터. csv 형식', required=True, type=openapi.TYPE_FILE)
    ...
    def postCurrentData(request):
        ...
        ##
        ## [요청 필드]
        ##   csvfile   : FILE    (필수) 워치에서 수집된 .csv 파일
        ##   userID    : STRING  (필수) 사용자 ID (ex. AA00) → DB users 컬렉션 authCode 필드와 매칭
        ##   battery   : INTEGER (필수) 배터리 잔량 (정수)
        ##   timestamp : STRING  (필수) Unix epoch 초단위 정수 문자열 (ex. 1672498800)
        ##                             ※ "2023-01-01" 같은 날짜 문자열 형식 불가 → 파싱 오류 발생
        ##
        ## [파일 저장 경로]
        ##   /mnt/ssd1/wearables/uploads/{userID}/{dd_mm_yyyy}/{filename}.csv
        ##   (timestamp를 변환해 날짜 폴더명 생성, 동일 파일 재전송 시 401 반환)
        ##
        ## [MongoDB 저장] devices 컬렉션에 아래 필드 insert
        ##   deviceID, battery, received_ts, last_battery_percent_date,
        ##   last_battery_percent_ts_kst, last_battery_percent_ts_gmt, date
        ##
        ## [응답 코드]
        ##   200 : 성공
...
# file2db
### 웨어러블 업로드 파일을 MongoDB에 적재하는 데일리 크론 스크립트

- 실행 파일: `file2db/watch_upload_daily.py`
- crontab 설정 (매일 00:30 실행, 경로는 환경에 맞게 수정):
```sh
30 0 * * * cd /path/to/file2db && /path/to/python watch_upload_daily.py
```
- 로그 경로: `file2db/logs/upload_logs/upload_log_daily_{날짜}.txt`

<details>
    <summary>watch_upload_daily.py : 전날 하루치 웨어러블 데이터 MongoDB 업로드</summary>

```python
    TARGET_DATE = date.today() - timedelta(days=1)      ## 대상 날짜: 실행일 기준 전날
    DATE_FOLDER = TARGET_DATE.strftime("%d_%m_%Y")      ## 탐색할 날짜 폴더명 (ex. 04_05_2026)

    def process_single_email(email, skip_emails, uploaded_emails):
        ## skip_emails(테스트 계정 목록)에 있으면 건너뜀
        ## uploaded_emails(당일 이미 처리 완료 목록)에 있으면 건너뜀

        BASE_PATH/{email}/{DATE_FOLDER}/ 경로에서 .csv 파일 탐색

        파일명으로 DATA_TYPE_MAP 키를 매칭하여 MongoDB 컬렉션 결정
        (Accelerometer, Gravity, Gyroscope → time/x/y/z 컬럼
         그 외 센서 → time/value 컬럼)

        헤더가 비정상(unnamed 또는 숫자)인 경우 컬럼명 강제 지정 후 재로드

        records를 BATCH_SIZE 단위로 MongoDB insert_many 업로드
        업로드 성공 시 파일을 UPLOADED_PATH로 이동

        처리 완료된 이메일은 EMAIL_LOG_FILE(json)에 기록 → 재실행 시 중복 방지
```

** `watch_upload.py` 에서 공통 상수 및 유틸 함수를 import하여 사용
** `SKIP_FILE` (csv): 테스트 계정 이메일 목록 (`test_mail_list` 컬럼)
** cron 실행 시 `cd /path/to/file2db` 로 작업 디렉토리를 지정해야 상대경로 import가 정상 동작함
