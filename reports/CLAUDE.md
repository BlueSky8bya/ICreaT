...
## 슬라이드 내용 규칙 (필수) — 정보 밀도 우선
발표자료는 **결론·수치·근거만** 담는다. 청중이 1초 안에 "무엇이 진행됐나"를 읽게.
- **메일 원문·인사말·따옴표 인용 금지.** "안녕하세요 ~입니다", "확인하였습니다" 같은 문장은 슬라이드에 넣지 마라. 메일에서 뽑을 건 **사실 1개**(수신 확정 / 합의된 스펙 / 요청 기한)뿐 — 그것만 한 줄 결론으로.
  - 나쁜 예: 회신 본문 4줄을 인용 박스로 박제.
  - 좋은 예: "담당자 수신 확인 — 전송 시각·파일 14개 1:1 일치" (한 줄).
- **증빙은 데이터로.** 메일이 근거면, 인용 대신 그 결과(파일 수·용량·시각·상태코드)를 표/지표로 보여라.
- 한 슬라이드 = 한 메시지. 본문 줄은 개조식, 군더더기·중복 제거.
- 원문이 필요하면 `mails/threads/`에 두고 슬라이드는 **링크/출처 한 줄**만. 슬라이드는 요약 산출물이지 원문 보관소가 아니다.
